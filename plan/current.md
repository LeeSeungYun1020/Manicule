...
| V5 | 서재 | 상태 변경은 V4 상태 API | 🚧 상태별 목록·빈 상태·책 상세/검색/스캔 이동·정렬·다중 상태 Preview 완료 | 진행률/완독일 오버레이 → 롱프레스 상태 변경·삭제·Undo |
...
1. V5 다음 PR로 읽는 중 진도율과 다 읽음 완독일 오버레이를 구현한다.
2. V5 상태별 서재 조회와 V7 공유 집계가 모두 머지됐으므로 V6 홈을 시작한다. V7 UI 완료는 기다리지 않는다.
3. V4 독서 상태 변경 API를 진행해 V5의 롱프레스 상태 변경 범위를 해소한다.
4. V1 검색 실행, V2 스캔 UI, V3 설정 UI, V7 통계 UI는 서로 독립적으로 진행할 수 있다.
5. V5 상태 변경은 V4의 상태 API가 머지된 뒤 연결한다.
6. I1은 각 V navigation PR이 머지되는 즉시 해당 destination을 조립하고, V3 테마 계약 뒤 앱 루트 테마를 연결한다.
