...
| V4 | 책 상세 | 없음 | 🚧 책 정보 조회·탭 유지·캐시 새로고침, 독서 상태 변경 API·미등록/UNSET 미선택 UI·최초 서재 등록·완독일·저장 실패 재시도·테스트 완료 | 별점·메모 인라인 저장 |
...
| V7 | 통계 | 없음 | 🚧 공유 집계·월요일 기준 공용 달력 선택·주별 고정 셀·높이/패딩 제약 대응 완료 | 통계 달력 조립·날짜별 기록 sheet → 오늘 목록 → 복합 차트·스크롤 테스트 |
...
1. V6 선행 작업인 서재 초기 탭 진입 계약이 머지되었으므로 홈 상태 조합·첫 사용자 화면을 진행한다. V5 정렬과 V7 UI 완료는 기다리지 않는다.
2. V4는 별점·메모 인라인 저장을 진행한다. 독서 상태 변경 API는 구현·검증 완료했으며, 머지 후 V5에서 재사용한다.
