...
> 선행 조건·변경 소유권·PR 분할의 기준. 기능 요구사항·모듈 경계·진행 현황은 진입점에서 각 담당 문서를 선택한다.

---

## 1. 구현 원칙

의미 충돌 위험이 큰 공용 모델·쿼리·Navigation route·다중 feature UI 계약을 먼저 확정한다.
...
1. 각 레인은 해당 `depends_on`이 충족되면 독립 레인의 완료를 기다리지 않고 착수할 수 있다.
2. `ManiculeIcons.kt`, `core:designsystem`의 `strings.xml`, Gradle 설정처럼 여러 레인이 단순 항목을 추가하는 파일은 공동 편집할 수 있다.
3. 기존 DAO에 서로 다른 신규 메서드를 추가하는 작업도 허용한다. 최종 머지자는 최신 `main` 기준으로 추가 항목과 import·정렬을 정리한다.
4. 같은 기능·Screen·ViewModel·공용 계약은 동시에 한 PR만 소유한다. 기존 공용 메서드의 의미와 API 시그니처는 선행 계약 PR 또는 지정된 소유 레인 한 곳에서 변경한다.
5. `ManiculeNavHost`를 여러 PR이 수정해야 한다면 선행 PR부터 `main`에 반영하며 직렬화한다. 병렬 구현한 화면의 조립도 통합 PR 하나가 소유한다.
6. `@Inject constructor`를 우선해 DI 모듈 공동 편집을 줄이되, 필요한 단순 바인딩 추가는 병렬 작업을 막는 이유로 삼지 않는다.

선행 PR이 `main`에 머지되면 후속 PR의 작업 브랜치에서 `git fetch origin main` 후 `git merge origin/main`으로 반영하고 충돌을 해결·검증한다.

### 2.2 PR 분할 기준

레인은 상위 의존성 단위이며 PR 단위가 아니다. 같은 레인에서도 위 소유권·선행 조건이 겹치지 않으면 PR을 병렬 구현할 수 있다.
...
트랜잭션·불변 조건을 임의로 나누거나 서로 다른 사용자 행동을 합치지 않는다.

다음 항목 중 둘 이상이 한 PR의 핵심 리뷰 대상이라면 분리를 우선한다.

1. 공용 API·모델·DB 스키마 또는 쿼리 의미 변경
2. coroutine 동시성·취소·lifecycle·WorkManager 스케줄링
3. ViewModel 상태 전이와 오류 복구
4. 공용 UI 컴포넌트의 동작 또는 레이아웃 변경
5. `app` navigation·inset·루트 테마 조립

#### 규모 가드레일

규모는 리뷰 범위 경고일 뿐이며, 트랜잭션·불변 조건을 나누거나 서로 다른 사용자 행동을 합치는 근거가 아니다.

| 구간 | 기준 |
|---|---|
| 권장 범위 | 비기계적 변경 5~15파일, 전체 diff 약 300~800줄 |
| 분할 검토 | 15파일 또는 800줄 초과 |
| 예외 사유 필수 | 20파일 또는 1,000줄 초과 |
| 계산 제외 | 생성된 Room schema, 반복 번역 리소스, history/current 문서 갱신 |
...
공통 UI 구현량이 크면 화면 상단·탐색, 피드백·상태, 통계 표시처럼 리뷰 가능한 컴포넌트 묶음별 PR로 나눈다.
...
| 계층 | 범위 |
|---|---|
| Data | 기존 UserPreferences Repository로 리마인더 설정을 저장한다. 발송 콘텐츠는 기존 Library Repository에서 최근 수정된 읽는 중 책을 최대 5권 조회한다. |
| Domain | `ReminderScheduler` 계약, 리마인더 on/off·시간 변경 UseCase, 읽는 중 책 제목 후보를 제공하는 `GetReminderContentUseCase`를 구현한다. Worker가 발송 시점에 후보를 조회하고, 후보가 3권 이상이면 무작위로 2권을 선택한다. 후보가 없으면 일반 메시지를 사용한다. |
...
2. 월요일 기준 공용 독서 달력 선택 계약과 UI 테스트
    - `ReadingCalendarGrid`는 선택 상태와 오늘 날짜를 호출부에서 받고, 선택 가능한 날짜에만 `clickable`과 클릭 semantics를 제공한다.
