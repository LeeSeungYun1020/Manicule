...
3. [current.md](current.md)는 작업 종료 시 한 번만 수정하며, 각 PR은 담당 레인의 상태와 다음 범위만 갱신한다.
4. 기존 DAO에 서로 다른 신규 메서드를 추가하는 작업도 허용한다. 최종 머지자는 최신 `main` 기준으로 추가 항목과 import·정렬을 정리한다.
5. 같은 기능을 두 레인에서 중복 구현하거나 기존 공용 메서드의 의미와 공용 API 시그니처를 동시에 변경하지 않는다.
6. 공용 API 변경은 선행 계약 PR 또는 아래에 지정된 소유 레인 한 곳에서만 수행한다. 다른 레인은 해당 변경이 머지된 뒤 rebase한다.
7. `@Inject constructor`를 우선해 DI 모듈 공동 편집을 줄이되, 필요한 단순 바인딩 추가는 병렬 작업을 막는 이유로 삼지 않는다.

### 2.2 PR 분할 기준

작업 레인은 병렬 배정과 상위 의존성 관리 단위이며 PR 단위가 아니다. 기능 규모에 따라 한 레인을 여러 PR로 분할한다. 같은 레인 안에서도 선행 작업과 변경 소유권이 겹치지 않는 PR은 병렬 구현할 수 있다. 다만 공용 Screen, ViewModel 또는 앱 조립 파일은 하나의 통합 PR만 소유한다.
...
| 분할 검토 | 비기계적 변경이 15파일 또는 전체 diff가 800줄을 넘는 경우 |
| 예외 사유 필수 | 비기계적 변경이 20파일 또는 전체 diff가 1,000줄을 넘는 경우 |
| 계산 제외 | 생성된 Room schema, 반복 번역 리소스, history/current 문서 갱신 |

규모를 맞추기 위해 하나의 트랜잭션이나 불변 조건을 인위적으로 나누지 않는다. 반대로 테스트가 많다는 이유로 서로 다른 사용자 행동을 한 PR에 합치지 않는다.

#### 병렬 PR 작업 계획

세분화된 PR 작업을 시작하기 전에 다음 내용을 작업 계획에서 명확히 한다.

1. 리뷰어가 판단할 행동 또는 불변 조건인 리뷰 초점
2. 의미 변경을 담당하는 기능·공용 API·주요 파일인 구현 범위와 제외 범위
3. 구현 또는 리뷰 전에 머지되어야 하는 선행 작업이나 의존 PR
4. 완료를 증명할 테스트 또는 화면 동작인 완료 기준

다음 조건을 모두 만족할 때만 PR을 병렬 구현한다.

1. 서로의 공용 API 의미 변경에 의존하지 않는다.
