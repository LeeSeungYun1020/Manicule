...
- **터치 타겟**: 최소 **48 × 48 dp**. 부족 시 `Modifier.minimumInteractiveComponentSize()` 활용. 인접 타겟 간 최소 8dp 간격. 독서 달력의 예외는 [기획](../plan.md#35-독서-통계)을 따른다.
...
- **상태별 UI**: 핵심 콘텐츠는 로딩 / 오류 / 빈 상태를 구분하고, 실패 시 사용자가 인지할 수 있는 오류 UI와 재시도·복구 동작을 제공한다. 보조 콘텐츠 실패는 핵심 흐름을 유지하도록 숨기거나 빈 상태와 동일하게 표현할 수 있지만, 상태 모델에서는 성공한 빈 결과와 오류를 구분하고 자동 재시도 / 수동 재시도 / 화면 재진입 시 재구독 중 하나 이상의 복구 경로를 둔다.
  - **Level 1 (전체 화면 에러)**: 데이터가 없는 초기 로드 실패 시 `ManiculeErrorState`로 화면 전체에 오류 안내와 재시도 액션 제공
  - **Level 2 (갱신 에러)**: 기존 데이터가 존재하는 상태에서 갱신 실패 시 기존 화면을 유지하고 `ManiculeSnackbarHost` 스낵바(재시도 포함)로 비간섭 안내 제공
  - **Level 3 (부분/추가 에러)**: 추가 페이지 로드나 독립 보조 섹션 로드 실패 시 해당 위치에 인라인 텍스트 및 재시도 액션 제공
...
- [ ] **터치 타겟**: 클릭 가능한 요소가 최소 `48x48dp` 크기를 보장하는가? 독서 달력은 [기획의 예외](../plan.md#35-독서-통계)를 적용한다.
